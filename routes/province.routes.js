...
router.get('/test', (req, res) => {
  res.status(200).json({
    success: true,
    message: 'Province API is working'
  });
});

// Routes that need to be defined before routes with ID params
...
router.get('/', connectProvinceController.getAllProvinces);

// Routes with ID params - these should come after other specific routes
router.get('/:id', connectProvinceController.getProvinceById);
router.get('/:id/connections', connectProvinceController.getProvinceConnections);
router.get('/:id/geojson', connectProvinceController.getProvinceGeoJSON);
router.get('/:id/polygon', connectProvinceController.getProvincePolygon);
